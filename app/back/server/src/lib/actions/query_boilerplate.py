...
class QueryEndpoint:
    ...
    def __init__(self, host: str = "http://127.0.0.1", port: Union[int, None] = 6000, delay: int = 2) -> None:
    ...
    def get_endpoint(self, path: str, content: Union[Dict[str, Any], None] = None, header: Union[Mapping[str, str], None] = None) -> requests.Response:
        ...
        if content is not None and header is None:
            return requests.get(f"{self._host}/{path}", json=content, timeout=self._delay)
        if content is None and header is not None:
            return requests.get(f"{self._host}/{path}", headers=header, timeout=self._delay)
        if content is not None and header is not None:
            return requests.get(f"{self._host}/{path}", json=content, headers=header, timeout=self._delay)
        return requests.get(f"{self._host}/{path}", timeout=self._delay)
    ...
    def post_endpoint(self, path: str, content: Union[Dict[str, Any], None] = None, header: Union[Mapping[str, str], None] = None) -> requests.Response:
        ...
        if path[0] == "/":
            path = path[1:]
        if content is not None and header is None:
            return requests.post(f"{self._host}/{path}", json=content, timeout=self._delay)
        if content is None and header is not None:
            return requests.post(f"{self._host}/{path}", headers=header, timeout=self._delay)
        if content is not None and header is not None:
            return requests.post(f"{self._host}/{path}", json=content, headers=header, timeout=self._delay)
        return requests.post(f"{self._host}/{path}", timeout=self._delay)
    ...
    def put_endpoint(self, path: str, content: Union[Dict[str, Any], None] = None, header: Union[Mapping[str, str], None] = None) -> requests.Response:
        ...
        if path[0] == "/":
            path = path[1:]
        if content is not None and header is None:
            return requests.put(f"{self._host}/{path}", json=content, timeout=self._delay)
        if content is None and header is not None:
            return requests.put(f"{self._host}/{path}", headers=header, timeout=self._delay)
        if content is not None and header is not None:
            return requests.put(f"{self._host}/{path}", json=content, headers=header, timeout=self._delay)
        return requests.put(f"{self._host}/{path}", timeout=self._delay)
    ...
    def patch_endpoint(self, path: str, content: Union[Dict[str, Any], None] = None, header: Union[Mapping[str, str], None] = None) -> requests.Response:
        ...
        if path[0] == "/":
            path = path[1:]
        if content is not None and header is None:
            return requests.patch(f"{self._host}/{path}", json=content, timeout=self._delay)
        if content is None and header is not None:
            return requests.patch(f"{self._host}/{path}", headers=header, timeout=self._delay)
        if content is not None and header is not None:
            return requests.patch(f"{self._host}/{path}", json=content, headers=header, timeout=self._delay)
        return requests.patch(f"{self._host}/{path}", timeout=self._delay)
    ...
    def delete_endpoint(self, path: str, content: Union[Dict[str, Any], None] = None, header: Union[Mapping[str, str], None] = None) -> requests.Response:
        ...
        if path[0] == "/":
            path = path[1:]
        if content is not None and header is None:
            return requests.delete(f"{self._host}/{path}", json=content, timeout=self._delay)
        if content is None and header is not None:
            return requests.delete(f"{self._host}/{path}", headers=header, timeout=self._delay)
        if content is not None and header is not None:
