"""_summary_
    This is the file in charge of storing the endpoints ready to be imported into the server class.
"""
import signal
...
class Endpoints:
    ...
    async def get_welcome(self, request: Request) -> Response:
        ...
        """
        token = self.rdi.bii.get_token_if_present(request)
        ...
        self.disp.log_debug(
            f"header = {self.rdi.json_header}", "get_welcome"
        )
        outgoing = HCI.success(
            content=body,
            content_type=CONST.CONTENT_TYPE,
            headers=self.rdi.json_header
        )
    ...
    async def post_login(self, request: Request) -> Response:
        ...
        return HCI.success(content=body, content_type=CONST.CONTENT_TYPE, headers=self.rdi.json_header)
